...
class Alarm:
    ...
    def get_expected_time(self):
        ...
        date_time_str = ''
        time = Alarm.match(self.text, alarm_utils.time_regex)
        date_time_str += time

        day_night = Alarm.match(self.text, alarm_utils.day_night_regex)
        date_time_str += day_night

        day = Alarm.match(self.text, alarm_utils.days_regex)
        date_time_str += day

        date = Alarm.match(self.text, alarm_utils.date_regex)
        # date_time_str += date

        period = Alarm.match(self.text, alarm_utils.period_regex)
